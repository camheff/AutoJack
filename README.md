...
- Updated code to **_display "Done"_** when **_Unplayed Sweeps Balance_** is **_less than $1_** so you dont accidentally play more than needed.
- **_Updated UI window_** to **_match the size_** of your Chrome browser and **_position_** itself **_directly below it_**
  - This is the **_intended position_**, the Action Icons are designed to be at roughly the **_same position_** as the actual game window
- **_UI window is now frameless_**. This helps save on screen space so you can stack it below the game window
  - If you dont want this or want the window to be moveable, run any of the start commands with ```:legacy``` appended to the back, like:

```bash
npm run start:luckyland:legacy
```
